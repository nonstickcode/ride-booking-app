...
export default async function handler(req, res) {
  try {
    ...
    const { data, error } = await supabase.from('NewBookingData').select('*').limit(1);

    if (error) {
      console.error('Keepalive query failed:', error);
      return res.status(500).json({ success: false, message: 'Keepalive failed' });
    }

    console.log('Keepalive query successful:', data);
    return res.status(200).json({ success: true, message: 'Keepalive successful' });
  } catch (err) {
